#使用gmssl库中的sm3算法，不过做了一点小的改动，见函数sm3_hash()
...
def sm3_hash(msg, new_v):
    ...
    """
     gmssl库中的sm3算法对应的sm3_hash函数只有msg一个参数。

    现在新增一个参数new_v，表示更新后的向量值。便于计算消息m'的哈希值hash2
    """
    len1 = len(msg)
    reserve1 = len1 % 64
    msg.append(0x80)
    ...
    group_count = round(len(msg) / 64) - 1  #加密次数比gmssl库中的sm3算法少一次
